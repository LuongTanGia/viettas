...
const PhieuThuTien = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (actionType === 'create' || actionType === 'edit') {
          const responseHM = await apis.ListHelperHangMucPTT(tokenLogin)
          ...
          const responseDT = await apis.ListHelperDoiTuongPTT(tokenLogin)
        ...
        if (actionType === 'edit') {
          ...
          const responseTT = await apis.ThongTinSuaPTT(tokenLogin, dataRecord.SoChungTu)
  ...
  useEffect(() => {
    const getChucNangQuyenHan = async () => {
      try {
        console.log('đi')
        const tokenLogin = localStorage.getItem('TKN')
        const response = await apis.ChucNangQuyenHan(tokenLogin, 'DuLieu_PTT')
  ...
  const getDSPTT = async () => {
    try {
      const tokenLogin = localStorage.getItem('TKN')
      const response = await apis.DanhSachPTT(tokenLogin, formKhoanNgay)
      if (response) {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'NgayCTu',
      align: 'center',
      render: (text) => <HighlightedCell text={moment(text).format('DD/MM/YYYY')} search={searchPTT} />,
      width: 150,
      sorter: (a, b) => {
...

    {
      ...
      render: (text) => (
        <div className={`text-end  ${text < 0 ? 'text-red-600 text-base' : text === 0 ? 'text-gray-300' : ''} `}>
          <HighlightedCell text={formatPrice(text, dataThongSo?.SOLESOTIEN)} search={searchPTT} />
        </div>
      ),
                            ...
                            .map((column, index) => {
                              ...
                              return (
                                ...
                                >
                                  {isNumericColumn ? (
                                    (() => {
                                      const total = Number(filteredPTT?.reduce((total, item) => total + (item[column.dataIndex] || 0), 0))
                                      return column.dataIndex === 'SoTien' ? (
                                        <Text strong className={total < 0 ? 'text-red-600 text-sm' : total === 0 ? 'text-gray-300' : ''}>
                                          {Number(filteredPTT.reduce((total, item) => total + (item[column.dataIndex] || 0), 0)).toLocaleString('en-US', {
