...
import { Select, Tooltip, Typography, Checkbox, Row, Button, Col, Spin, Input, Empty } from 'antd'
...
const DoanhSoBanHangKho = () => {
  ...
  useEffect(() => {
    ...
    const key = Object.keys(dataNXT ? dataNXT[0] : [] || []).filter((key) => key)
    setOptions(key)
  }, [selectVisible])

  console.log('kho', dataKhoHang)
          ...
          {!isLoading ? (
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="NhapXuatTonKho" id="my-table">
                <Table param={filteredHangHoa} columName={nameColumsDSBHKHO} height={'setHeight'} selectMH={[1]} textSearch={searchHangHoa} typeTable={'DSBH'} hiden={hiddenRow} />
              </div>
            </div>
          )}
        </>
