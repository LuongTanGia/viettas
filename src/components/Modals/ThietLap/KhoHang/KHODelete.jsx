...
const KHODelete = ({ close, dataKHO, loadingData, setTargetRow }) => {
  const TokenAccess = localStorage.getItem('TKN')
  const handleDelete = async () => {
    try {
      const response = await categoryAPI.XoaKhoHang(dataKHO?.MaKho, TokenAccess)
      ...
      <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col min-w-[40rem] min-h-[8rem] bg-white  p-2 rounded shadow-custom overflow-hidden">
        <div className="flex flex-col gap-2 p-2">
          <div className="flex gap-2">
            ...
            <p className="text-blue-700 font-semibold uppercase">Xóa dữ liệu - Kho Hàng</p>
          </div>
          <div className="flex flex-col gap-2 border-2 p-3 font-bold text-lg">
            <div className="flex gap-1">
              ...
              <p className="text-red-600">{dataKHO?.MaKho}</p>
