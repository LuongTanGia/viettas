...
const XDCCreate = ({ close, loadingData, setTargetRow }) => {
  ...
  const handleCreate = async (isSave = true, isPrint = true) => {
    if (!XDCForm?.MaKho) {
      setErrors({
        MaKho: XDCForm?.MaKho ? '' : 'Kho không được trống',
      })
      return
    }
    try {
      ...
      if (response.data.DataError == 0) {
        isPrint ? handlePrint() : isSave ? (toast.success('Tạo thành công'), setXDCForm([]), setSelectedRowData([])) : (close(), toast.success('Tạo thành công'))
        loadingData()
