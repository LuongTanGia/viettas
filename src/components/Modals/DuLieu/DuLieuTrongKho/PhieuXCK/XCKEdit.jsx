...
const XCKEdit = ({ close, dataXCK, loadingData, setTargetRow }) => {
  ...
  const [XCKForm, setXCKForm] = useState(() => {
    return dataXCK ? { ...dataXCK } : innitProduct
  })
  useEffect(() => {
  ...
  useEffect(() => {
    if (dataXCKView?.DataDetails) {
      setSelectedRowData([...dataXCKView.DataDetails])
    }
  }, [dataXCKView])
    ...
    try {
      ...
      if (newData?.length > 0) {
        if (isAdd) {
          ...
          const response = await categoryAPI.XCKEdit(
            { SoChungTu: dataXCK?.SoChungTu, Data: { ...XCKForm, GhiChu: XCKForm?.GhiChu?.trim(), NgayCTu: dayjs(XCKForm?.NgayCTu).format('YYYY-MM-DD'), DataDetails: newData } },
            TokenAccess,
          )
          if (response.data.DataError == 0) {
      ...
      {!isLoading ? (
        ...
        <>
          ...
          <div>
            {isShowModal &&
              (actionType === 'print' ? (
                <XCKPrint close={() => close()} dataPrint={{ ...XCKForm }} />
              ) : (
