...
const HMTDelete = ({ close, dataHMT, loadingData, setTargetRow }) => {
  const TokenAccess = localStorage.getItem('TKN')
  const handleDelete = async () => {
    try {
      const response = await categoryAPI.XoaHangMucThu(dataHMT?.Ma, TokenAccess)
        ...
        <div className="flex flex-col  gap-2">
          ...
          <div className="flex flex-col gap-2 border-1 border-gray-400 p-4  text-lg">
            <div className="flex gap-1">
              ...
              <Tooltip title={dataHMT?.Ma} color="blue">
                <p className="text-red-600 truncate">{dataHMT?.Ma}</p>
              </Tooltip>
