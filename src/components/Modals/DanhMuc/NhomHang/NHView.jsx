...
const NHView = ({ close, dataNH }) => {
  const TokenAccess = localStorage.getItem('TKN')
  const [dataNHView, setDataNHView] = useState('')
  const [isLoading, setIsLoading] = useState(false)

  useEffect(() => {
    const handleView = async () => {
      try {
        const response = await categoryAPI.InfoNhomHang(dataNH?.MaNhom, TokenAccess)
        if (response.data.DataError == 0) {
          setDataNHView(response.data.DataResult)
          setIsLoading(true)
                ...
                <div className="flex flex-col gap-2 border-2 px-3 py-2.5">
                  <div className="flex items-center gap-2">
                    <div className="flex items-center gap-1 w-[60%]">
                      ...
                      <input type="text" value={dataNHView?.MaNhom || ''} className="px-2 w-full resize-none rounded border outline-none text-sm truncate" readOnly />
                    </div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <input type="text" value={dataNHView?.TenNhom || ''} className="px-2 w-full resize-none rounded border outline-none text-sm truncate" readOnly />
                  </div>
                  <div className="flex items-center gap-1">
                    <label className=" whitespace-nowrap  min-w-[90px] text-sm flex justify-end">Ghi chú</label>
                    <input type="text" value={dataNHView?.GhiChu || ''} className="px-2 w-full resize-none rounded border outline-none text-sm truncate" readOnly />
                  </div>
                    ...
                    <div className="flex gap-2">
                      <div className="flex items-center gap-2 whitespace-nowrap">
                        ...
                        <Tooltip title={dataNHView?.NguoiTao} color="blue">
                          <input value={dataNHView?.NguoiTao || ''} className="w-full px-2 rounded resize-none border outline-none text-[1rem] truncate" readOnly />
                        </Tooltip>
                      </div>
                      <div className="flex items-center gap-2 whitespace-nowrap ">
                        <label className=" text-sm">Lúc</label>
                        <Tooltip title={moment(dataNHView?.NgayTao)?.format('DD/MM/YYYY HH:mm:ss')} color="blue">
                          <input
                            type="text"
                    ...
                    <div className="flex gap-2">
                      <div className="flex items-center gap-2 whitespace-nowrap">
                        ...
                        <Tooltip title={dataNHView?.NguoiSuaCuoi} color="blue">
                          <input value={dataNHView?.NguoiSuaCuoi || ''} className="w-full px-2 rounded  resize-none border outline-none text-[1rem] truncate" readOnly />
                        </Tooltip>
                      </div>
                      <div className="flex items-center gap-2 whitespace-nowrap ">
                        <label className=" text-sm">Lúc</label>
