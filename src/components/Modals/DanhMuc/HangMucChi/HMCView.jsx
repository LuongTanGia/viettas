...
const HMCView = ({ close, dataHMC }) => {
  const TokenAccess = localStorage.getItem('TKN')
  const [dataHMCView, setDataHMCView] = useState('')
  const [isLoading, setIsLoading] = useState(false)

  useEffect(() => {
    const handleView = async () => {
      try {
        const response = await categoryAPI.InfoHangMucChi(dataHMC?.Ma, TokenAccess)
        if (response.data.DataError == 0) {
          setDataHMCView(response.data.DataResult)
          setIsLoading(true)
            ...
            <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col bg-white p-2 rounded shadow-custom overflow-hidden">
              <div className="flex flex-col gap-2 py-1 px-2 md:w-[80vw] lg:w-[60vw] xl:w-[45vw]">
                <div className="flex gap-2">
                  ...
                  <p className="text-blue-700 font-semibold uppercase">Thông tin - Hạng Mục Chi Tiền</p>
                </div>
                <div className="flex flex-col gap-2 border-gray-400 border-1 px-3 py-2.5">
                  <div className="flex items-center gap-1">
                    <label className=" whitespace-nowrap required min-w-[50px] text-sm flex justify-end">Mã</label>
                    <input type="text" value={dataHMCView?.Ma || ''} className="px-2 h-6 w-[30%] resize-none rounded-[3px] border outline-none text-sm truncate" disabled />
                  </div>
                  <div className="flex items-center gap-1">
                    <label className=" whitespace-nowrap required min-w-[50px] text-sm flex justify-end">Tên</label>
                    <input type="text" value={dataHMCView?.Ten || ''} className="px-2 h-6 w-full resize-none rounded-[3px] border outline-none text-sm truncate" disabled />
                  </div>
                  <div className="flex items-center gap-1">
                    <label className=" whitespace-nowrap  min-w-[50px] text-sm flex justify-end">Ghi chú</label>
                    <input type="text" value={dataHMCView?.GhiChu || ''} className="px-2 h-6 w-full resize-none rounded-[3px] border outline-none text-sm truncate" disabled />
                  </div>
                  <div className="grid grid-cols-1 mt-1 gap-2 px-2 py-3 rounded border-black-200 ml-[55px] relative border-[0.125rem]">
                    ...
                    <div className="flex gap-2 justify-center">
                      <div className="flex items-center gap-1.5 whitespace-nowrap">
                        ...
                        <Tooltip title={dataHMCView?.NguoiTao} color="blue">
                          <input
                            value={dataHMCView?.NguoiTao || ''}
                    ...
                    <div className="flex gap-2 justify-center">
                      <div className="flex items-center gap-1 whitespace-nowrap">
                        ...
                        <Tooltip title={dataHMCView?.NguoiSuaCuoi} color="blue">
                          <input
                            value={dataHMCView?.NguoiSuaCuoi || ''}
                            className="2xl:w-[20vw] xl:w-[15vw] lg:w-[20vw] md:w-[24vw] px-2 h-6 rounded-[3px] resize-none border outline-none text-sm truncate"
                            disabled
                          />
                        </Tooltip>
