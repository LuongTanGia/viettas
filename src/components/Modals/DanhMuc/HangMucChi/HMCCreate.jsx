...
const HMCCreate = ({ close, loadingData, setTargetRow }) => {
  ...
  const handleCreate = async (isSave = true) => {
    if (!HMCForm?.Ma?.trim() || !HMCForm?.Ten?.trim()) {
      setErrors({
        Ma: HMCForm?.Ma?.trim() ? null : 'Mã không được trống',
        ...
        <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col bg-white p-2 rounded shadow-custom overflow-hidden">
          <div className="flex flex-col gap-2 py-1 px-2 md:w-[80vw] lg:w-[60vw] xl:w-[45vw]">
            <div className="flex gap-2">
              ...
              <p className="text-blue-700 font-semibold uppercase">Thêm - Hạng Mục Chi Tiền</p>
            </div>
              ...
              <div className="grid grid-cols-1 mt-1 gap-2 px-2 py-2.5 rounded border-black-200 ml-[55px] relative border-[0.125rem]">
                ...
                <div className="flex gap-2 justify-center">
                  <div className="flex items-center gap-1.5 whitespace-nowrap">
                    ...
                    <input className="2xl:w-[20vw] xl:w-[15vw] lg:w-[20vw] md:w-[24vw] px-2 rounded-[3px] resize-none border outline-none text-sm truncate" disabled />
                  </div>
