...
import { nameColumsDoanhSoBanHangHH } from '../util/Table/ColumnName'
...
function DSBHHH() {
  ...
  useEffect(() => {
    ...
    const key = Object.keys(data[0] || []).filter((key) => key !== 'ThongTinHangHoa')
    setOptions(key)
  }, [selectVisible])
    ...
    <>
      <div className="flex justify-between relative">
        <div className=" flex items-center gap-x-4 ">
          <h1 className="text-xl font-black uppercase">Doanh số bán hàng (hàng hóa) </h1>
        </div>
                          ...
                          options?.map((item, index) => (
                            <Col span={10} key={(item, index)}>
                              <Checkbox value={item} checked={true}>
                                {nameColumsDoanhSoBanHangHH[item]}
                              </Checkbox>
                            </Col>
                          ))
                        ) : (
      ...
      <div id="my-table">
        <Tables loadingSearch={tableLoad} param={data} columName={nameColumsDoanhSoBanHangHH} height={'setHeight'} selectMH={[1]} typeTable={'DSBH'} hiden={hiddenRow} />
      </div>
    </>
  )
}

export default DSBHHH
