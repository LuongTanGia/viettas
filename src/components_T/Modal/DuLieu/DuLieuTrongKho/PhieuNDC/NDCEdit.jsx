...
const NDCEdit = ({ close, dataNDC }) => {
  ...
  console.log(dataNDCView)
  const handleEdit = async (e) => {
    e.preventDefault()
    try {
      console.log('1', { SoChungTu: dataNDC?.SoChungTu, Data: { ...NDCForm, NgayCTu: dayjs(valueDate).format('YYYY-MM-DDTHH:mm:ss'), DataDetails: dataNDCView.DataDetails } })
      const response = await categoryAPI.NDCEdit(
        { SoChungTu: dataNDC?.SoChungTu, Data: { ...NDCForm, NgayCTu: dayjs(valueDate).format('YYYY-MM-DDTHH:mm:ss'), DataDetails: dataNDCView.DataDetails } },
        TokenAccess,
      )
      if (response.data.DataError == 0) {
